...
export function AgeGroupPicker({
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-foreground text-sm font-medium">Your age group</p>
      <div
        role="group"
      ...
      <p className="text-muted-foreground text-xs">
        {value === "all"
          ? "Ranking uses overall earnings. Pick an age band to optimise for your stage."
          : `Career score and avg salary use the ${AGE_GROUPS.find((g) => g.id === value)?.label} earnings band.`}
      </p>
    </div>
  );
}
