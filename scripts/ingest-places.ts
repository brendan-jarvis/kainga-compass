 ...
 *
 * Validates the committed places fixture set and regenerates simplified
 * boundary GeoJSON. Wire live parsers when ready:
 *   - MBIE TA / region bond CSVs (district view)
 *   - MBIE Market Rent API — SA2 (city/town view)
 *   - HUD Local Housing Statistics XLSX (district)
 *   - Stats NZ Urban Rural geography
 *
...
async function main() {
  ...
  const metadata = {
    ...
    sources: [
      ...
      "Stats NZ Urban Rural 2023/2025 — settlement geography for city/town units",
      "Stats NZ subnational population estimates (urban rural) — density proxies",
      "Indicative settlement splits where TA data alone would mask local markets",
    ],
    cityCount,
    regionCount,
    notes:
      "Cities & towns are settlement-scale. Districts are territorial authorities. Settlement splits for multi-town TAs are approximate until SA2/Market Rent pipelines are wired.",
  };
  ...
  console.log(
    `✓ ingest:places — ${cityCount} cities/towns + ${regionCount} districts → src/data/places/`,
  );
}
